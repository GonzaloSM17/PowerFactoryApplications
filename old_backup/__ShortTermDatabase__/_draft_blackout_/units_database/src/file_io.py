...
class FileIO():
    ...
    def _read_files_scenario_units(self):
        ...
        _data_io.load_dataframe(sheet_name="PFV", header=[0, 1])
        _data_io.flatten_columns()
        _data_io.dataframe.columns = ["unidad_powerfactory"]

        self.fv_units = _data_io.dataframe

        _data_io.load_dataframe(sheet_name="PE", header=[0, 1])
        _data_io.flatten_columns()
        _data_io.dataframe.columns = ["unidad_powerfactory"]

        self.eo_units = _data_io.dataframe

        _data_io.load_dataframe(sheet_name="BESS", header=[0, 1])
        _data_io.flatten_columns()
        _data_io.dataframe.columns = ["unidad_powerfactory"]

        self.bess_units = _data_io.dataframe
    
    def _merge_units(self):

        _fv_units   = self.fv_units
        _eo_units   = self.eo_units
        _bess_units = self.bess_units

        _units    = self.units

        _fv_merge = ds.fuzzy_merge(
            df_left=_fv_units, 
            df_right=_units, 
        ...
        self.fv_merge = _fv_merge

        _eo_merge = ds.fuzzy_merge(
            df_left=_eo_units, 
            df_right=_units, 
            left_on="unidad_powerfactory", 
            right_on="nombre_infotecnica",
        ...
        self.eo_merge = _eo_merge

        _bess_merge = ds.fuzzy_merge(
            df_left=_bess_units, 
            df_right=_units, 
            left_on="unidad_powerfactory", 
            right_on="nombre_infotecnica",
        ...
        self.bess_merge = _bess_merge
    
    def _merge_ids(self):

        _plants = self.plants

        _fv_merge   = self.fv_merge
        _eo_merge   = self.eo_merge
        _bess_merge = self.bess_merge

        __fv_merge = _fv_merge.merge(
            _plants,
            on="nombre_central",
            how="left"
        )

        self.fv_merge = __fv_merge

        __eo_merge = _eo_merge.merge(
            _plants,
            on="nombre_central",
            how="left"
        )

        self.eo_merge = __eo_merge

        __bess_merge = _bess_merge.merge(
            _plants,
            on="nombre_central",
            how="left"
        )

        self.bess_merge = __bess_merge
    
    def _export_data(self, filename="unidades_infotecnica_blackout_V1.xlsx"):

        try:
            with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
                self.fv_merge.to_excel(writer, sheet_name="PFV", index=False)                
                self.eo_merge.to_excel(writer, sheet_name="PE", index=False)
                self.bess_merge.to_excel(writer, sheet_name="BESS", index=False)
