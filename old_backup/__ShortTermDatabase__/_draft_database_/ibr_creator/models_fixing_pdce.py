...
class ModelFixer:
    ...
    def _variations_extractions(self):
        ...
        _variations_folders = self.app.GetProjectFolder("scheme").GetContents(
            "*.IntFolder"
        )
    ...
    def _pmgds_mod(self):
        ...
        for _pmgd in _pmgds:
            ...
            row_0 = iq_.GetAttribute("M:0")
            row_3 = iq_.GetAttribute("M:3")

            row_0[1] = 1
            row_3[1] = -1

            iq_.SetAttribute("M:0", row_0)
            iq_.SetAttribute("M:3", row_3)

    def _cte_impendance(self):

        generator_1 = self.app.GetCalcRelevantObjects("PFV *.ElmGenstat")
        generator_2 = self.app.GetCalcRelevantObjects("PE *.ElmGenstat")
        generator_3 = self.app.GetCalcRelevantObjects("BESS *.ElmGenstat")

        generators = generator_1 + generator_2 + generator_3

        for _generator in generators:

            if _generator.c_pmod is None:
                pass

            else:

                capacity = _generator.sgn * _generator.ngnum

                if capacity <= 10:
                    _generator.iSimModel = 3
...
class DSLFixer:
    ...
    def _dsl_extractions(self):
        ...
        for _dsl in _dsls:
            if any(
                name in _dsl.fold_id.loc_name
                for name in ["P/S PFV", "P/S PE", "P/S BESS"]
...
if __name__ == "__main__":
    ...
    # desktop.Show()
    print(
        f"Time taken to process units: {int((time.time() - start_time) // 60)} minutes and {(time.time() - start_time) % 60:.2f} seconds"
    )
